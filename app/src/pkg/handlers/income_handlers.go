...
func EditIncome(w http.ResponseWriter, r *http.Request) {
	...
	if loggedIn {
		...
		if householdId > 0 {
			...
			app.Session.Put(r.Context(), "warning", "You can't edit income without a household.")
			http.Redirect(w, r, "/create-a-household", http.StatusSeeOther)
			return
		}

	} else {
		app.Session.Put(r.Context(), "warning", "You have to be logged in to edit an income")
		http.Redirect(w, r, "/", http.StatusSeeOther)
	}
}
