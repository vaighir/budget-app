...
func AddUpcomingExpense(w http.ResponseWriter, r *http.Request) {
	...
	if loggedIn {
		...
		if householdId > 0 {
			...
			app.Session.Put(r.Context(), "warning", "You can't add upcoming expense without a household.")
			http.Redirect(w, r, "/create-a-household", http.StatusSeeOther)
			return
		}

	} else {
		app.Session.Put(r.Context(), "warning", "You have to be logged in to add an upcoming expense")
...
func EditUpcomingExpense(w http.ResponseWriter, r *http.Request) {
	...
	if loggedIn {
		...
		if householdId > 0 {
			...
			app.Session.Put(r.Context(), "warning", "You can't edit upcoming expense without a household.")
			http.Redirect(w, r, "/create-a-household", http.StatusSeeOther)
			return
		}

	} else {
		app.Session.Put(r.Context(), "warning", "You have to be logged in to edit upcoming expense")
		http.Redirect(w, r, "/", http.StatusSeeOther)
	}
}
...
func DeleteUpcomingExpense(w http.ResponseWriter, r *http.Request) {
	...
	if loggedIn {
		...
		app.Session.Put(r.Context(), "warning", "You have to be logged in to delete upcoming expense")
		http.Redirect(w, r, "/", http.StatusSeeOther)
	}
}
