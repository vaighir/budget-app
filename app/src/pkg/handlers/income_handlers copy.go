...
func AddFund(w http.ResponseWriter, r *http.Request) {
	...
	if loggedIn {
		...
		if householdId > 0 {
			...
			app.Session.Put(r.Context(), "warning", "You can't add fund without a household.")
			http.Redirect(w, r, "/create-a-household", http.StatusSeeOther)
			return
		}

	} else {
		app.Session.Put(r.Context(), "warning", "You have to be logged in to add an fund")
...
func EditFund(w http.ResponseWriter, r *http.Request) {
	...
	if loggedIn {
		...
		if householdId > 0 {
			...
			app.Session.Put(r.Context(), "warning", "You can't edit fund without a household.")
			http.Redirect(w, r, "/create-a-household", http.StatusSeeOther)
			return
		}

	} else {
		app.Session.Put(r.Context(), "warning", "You have to be logged in to edit an fund")
		http.Redirect(w, r, "/", http.StatusSeeOther)
	}
}
